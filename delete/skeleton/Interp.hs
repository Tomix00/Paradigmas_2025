...
interp_resize 1 img x w h = img x w h
interp_resize 2 img x w h = img x (w V.+ x) (h V.+ x)
interp_resize 3 img x w h = img x (w V.+ x V.+ x) (h V.+ x V.+ x)
...
interp (Rotar b) = interp_rotar (interp b)
interp (Apilar f1 f2 b1 b2) = interp_apilar f1 f2 (interp b1) (interp b2)
interp (Encimar b1 b2) = interp_encimar (interp b1) (interp b2)
interp (Resize n b) = interp_resize n (interp b)
